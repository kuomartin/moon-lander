...
def train(timesteps, resume=False, model_path="ppo_pixel_moon_lander"):
    print("Creating environment...")
    # 建立多個平行的環境來加速訓練 (Vectorized Environments)
    # 並指定 render_mode 為 rgb_array 給 CNN 觀測
    env_kwargs = {"render_mode": "rgb_array"}
    env = make_vec_env("PixelMoonLander-v0", n_envs=4, env_kwargs=env_kwargs)
    ...
    if resume:
        ...
        try:
            # 載入現有模型，並將環境綁定上去
            model = PPO.load(
                model_path, env=env, tensorboard_log="./ppo_lander_tensorboard/"
            )
    ...
    if not resume:
        ...
        model = PPO(
            ...
            env,
            verbose=1,
            tensorboard_log="./ppo_lander_tensorboard/",
            learning_rate=3e-4,
            n_steps=1024,
            batch_size=64,
            ent_coef=0.01,  # 新增：鼓勵模型多做隨機探索，避免太快陷入「馬上墜毀」的次優解
        )
    ...
    eval_env = make_vec_env("PixelMoonLander-v0", n_envs=1, env_kwargs=env_kwargs)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train or test a Stable Baselines3 agent for PixelMoonLander."
    )
    parser.add_argument(
